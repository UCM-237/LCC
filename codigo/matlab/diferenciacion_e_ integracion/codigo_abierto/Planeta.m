...
%empezar probando con Planeta(1,[1 0],[0 0.9],100) da orbitas eliptica.
...
while t<=tfinal
    t=t+dt;
v(1)=v(1)-m*dt*x(1)/(x(1)^2+x(2)^2)^(3/2); %integro la componente (1) de la fuerza
v(2)=v(2)-m*dt*x(2)/(x(1)^2+x(2)^2)^(3/2); %integro la componente (2) de la fuerza
x(1)=x(1)+v(1)*dt; %integro velocidades y obtengo posiciones
x(2)=x(2)+v(2)*dt;
delete(h)
h=plot(x(1),x(2),'o','MarkerFaceColor','b','MarkerSize',5); 
hold on
